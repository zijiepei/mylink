...
public class ArraySort {
    ...
    public static void heapSort3(int[] array) {
        ...
        for(int i = 0;i < array.length;i++) {
            swap(array,0,heapsize - 1);
            heapsize--;
            shiftDown3(array,heapsize,0);
        }
    }

    private static void shiftDown3(int[] array, int heapsize, int index) {
        int parent = index;
        int child = 2 * parent + 1;
    ...
    private static void creatHeap3(int[] array) {
        for(int i = (array.length - 1 - 1) / 2;i >= 0;i--) {
            shiftDown3(array,array.length,i);
        }
    }
    ...
    public static void heapSort4(int[] array) {
        creatHeap4(array);
        int heapsize = array.length;
        for(int i = 0;i < array.length;i++) {
            swap(array,0,heapsize - 1);
            heapsize--;
            shiftDown4(array,heapsize,0);
        }
    }

    private static void creatHeap4(int[] array) {
        for(int i = 0;i < array.length;i++) {
            shiftDown4(array,array.length,i);
        }
    }

    private static void shiftDown4(int[] array, int length, int index) {
        int parent = index;
        int child = 2 * parent + 1;
        while (child < length) {
            if(child + 1 < length && array[child + 1] < array[child]) {
    ...
    public static void slectSort4(int[] array) {
        for(int bound = 0;bound < array.length;bound++) {
            for(int cur = bound;cur < array.length;cur++) {
                if(array[cur] < array[bound]) {
    ...
    public static void quickSort4(int[] array) {
        quickSortHelper4(array,0,array.length - 1);
    }

    private static void quickSortHelper4(int[] array, int left, int right) {
        if(left >= right) {
            return;
        }
        int index = patition4(array,left,right);
        quickSortHelper4(array,left,index - 1);
        quickSortHelper4(array,index + 1,right);
    }

    private static int patition4(int[] array, int left, int right) {
        int i = left;
        int j = right;
        int base = array[right];
        while (i < j) {
    ...
    public static void shellSort4(int[] array) {
        int gap = array.length / 2;
        while (gap > 1) {
            insertSortgap4(array,gap);
            gap = gap / 2;
        }
        insertSortgap4(array,1);
    }

    private static void insertSortgap4(int[] array, int gap) {
        for(int bound = gap;bound < array.length;bound++) {
          int tmp = array[bound];
          int cur = bound - gap;
          for(cur = bound - gap;cur >= 0;cur = cur - gap) {
    ...
    public static void mergeSort4(int[] array) {
        mergeSortHelper4(array,0,array.length);
    }

    private static void mergeSortHelper4(int[] array, int left, int right) {
        if(right - left <= 1) {
            return;
        }
        int mid = (left + right) / 2;
        mergeSortHelper4(array,left,mid);
        mergeSortHelper4(array,mid,right);
        merge(array,left,mid,right);
    }
